"""Random Forest - Bagging mit zufälliger Merkmalsteilmenge je Schnitt - interaktive Konzept-Demo
...
lässt an jedem Schnitt nur eine zufällige Teilmenge der Merkmale zur Wahl (mtry) und entkoppelt die Bäume dadurch, ohne die Daten anzufassen.
...
VERDICT_TEXT = {
    ...
    "decorrelated_no_gain": "ℹ️ **Weniger korrelierte Bäume, aber kein besserer Testfehler als bei Bagging** - Dekorrelation allein genügt nicht, wenn die Schnitte dadurch zu schwach werden.",
...
unterschiedlicher Stichproben korreliert, und Mitteln hilft weniger (gemessen in bagging-demo). **Random Forest** (Breiman 2001) behebt das an der Wurzel des Problems: an **jedem** Schnitt jedes Baums
...
2. **Der Unterschied:** an jedem Knoten wird zuerst eine zufällige Teilmenge von **mtry** der d Merkmale gezogen (ohne Zurücklegen, ein frischer Zug je Knoten) - nur unter diesen wird der beste Schnitt gesucht. Ein Merkmal, das an diesem Knoten nicht gezogen wurde, kommt dort nicht in Frage, egal wie stark es wäre.
3. **mtry = alle Merkmale** macht daraus wieder exakt Bagging (Kreuzprobe, siehe Verifikation) - **mtry = 1** macht jeden Schnitt fast zufällig.
...
with st.sidebar:
    ...
    mtry = st.slider(f"mtry (Merkmale je Schnitt, von {d_now})", 1, mtry_hi, key="mtry_slider",
                     help=f"Wie viele der {d_now} Merkmale an jedem Schnitt überhaupt zur Wahl stehen. 1 = fast zufällig (schwach, aber entkoppelt), {d_now} = alle Merkmale = Bagging. "
...
st.caption("Gini-Wichtigkeit (blau) summiert die Unreinheitsabnahme aller Schnitte eines Merkmals - sie bevorzugt Merkmale mit vielen möglichen Schwellen, auch wenn sie nur Rauschen sind. Permutationswichtigkeit (grün) mischt "
...
if st.session_state.get("imp_on"):
    ...
    st.caption(f"Mittel über sechs Datensätze: bei {int(n_noise)} Rauschmerkmalen bekommt die Gini-Wichtigkeit **{irows['gini_noise_share']:.1%}** ihrer Summe auf reine Rauschmerkmale verteilt - obwohl die nichts mit dem Ziel zu tun haben. "
               ...
               "Grund: mehr mögliche Schwellen (stetige Merkmale) geben mehr Gelegenheit für einen zufällig guten Schnitt - Gini zählt das als Wichtigkeit mit, Permutation nicht (Strobl u. a. 2007).")
...
|---|---|---|
| **mtry braucht einen Mittelweg** | Zu klein (mtry = 1): jeder Schnitt ist fast zufällig, die Bäume sind entkoppelt, aber einzeln schwach - der Testfehler kann schlechter sein als bei Bagging (gemessen oben). Zu groß (mtry = alle): wieder Bagging, mit seiner Korrelation. | Experiment / Kreuzvalidierung für mtry |
...
| **Gini-Wichtigkeit bleibt verzerrt, auch gemittelt** | Sie bevorzugt Merkmale mit vielen möglichen Schwellen (stetige Merkmale, hohe Kardinalität) - das ist ein Artefakt der Schnittsuche, kein Verdienst des Merkmals (gemessen oben). | Permutationswichtigkeit |
...
with st.expander("📐 Mathematische Formulierung"):
    st.markdown(
        r"""
**Random Forest.** Wie Bagging: $B$ Bäume $T_1,\dots,T_B$ auf Bootstrap-Stichproben, $\hat f(x)=\frac1B\sum_b T_b(x)$. Der einzige Unterschied: an jedem Knoten $t$ mit Merkmalsmenge $\{1,\dots,d\}$ wird vor der Schnittsuche eine
Teilmenge $M_t\subset\{1,\dots,d\}$, $|M_t|=\texttt{mtry}$, ohne Zurücklegen gezogen; der Gewinn $\Delta(j,s)$ (wie in cart-demo) wird nur für $j\in M_t$ maximiert. Für $\texttt{mtry}=d$ ist $M_t=\{1,\dots,d\}$ immer - identisch mit Bagging.
...
Kleineres mtry senkt $\rho$ (jeder Baum sieht andere Merkmale an jedem Knoten), erhöht aber typischerweise $\sigma^2$ je Baum (schwächere Schnitte) - das Produkt hat ein Minimum, das gemessene "beste mtry".
